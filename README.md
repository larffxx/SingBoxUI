...
## Создание профиля из ссылки

Профиль можно создать из ссылки провайдера (`vless://`, `vmess://`, `trojan://`,
`ss://`, `hysteria2://`, `tuic://`) — кнопка «Из ссылки» на экране профилей.
Ссылка описывает один outbound, поэтому конфигурация генерируется целиком: за
основу берётся один из шаблонов-стартеров (VPN с TUN-устройством или локальный
порт без TUN), его сервер-заглушка заменяется разобранной ссылкой, а `route.final`,
DNS и правила перенаправляются на неё. Несколько ссылок попадают в группу
`urltest`, чтобы профиль работал без выбора сервера вручную. Разбор и генерация
живут в бэкенде (`internal/app/share`, `internal/app/profiles`), окно только
показывает результат разбора и предупреждения о параметрах, которые sing-box не
поддерживает. Решение описано в `docs/adr/014-share-link-profiles.md`.
...
Во вкладке «Routing» профиля есть карточка «Приложения»: список программ этой машины, у
каждой две кнопки — «Через VPN» и «Напрямую». Состояние по умолчанию не «не задано», а
то, что программа и так делает: его определяет `route.final` профиля. Если весь трафик
идёт через VPN (обычный случай), каждая программа показана как «через VPN», а правило —
исключение, которое ставит кнопка «Напрямую»; если финал ведёт напрямую, наоборот —
правило вводит программу в туннель. Кнопка пишет обычное правило маршрутизации, которое
выбирает программу по процессу, а не по адресу: на macOS это выражение по пути приложения
(`.app`), на Windows — имя файла программы (`process_name`), поэтому правило переживает
обновление, которое переносит программу в другую папку. Смена направления у одной
программы не трогает остальные: программа выходит из правил, которые её выбирают, и
получает своё. Список на Windows собирается из запущенных программ и ярлыков меню
«Пуск»; сама Windows, её установщики и сама SingBoxUI в него не попадают. Показываются
первые пять строк, дальше — по кнопке; целое дерево каталогов (например, игровую
библиотеку) удобнее исключать условием `process_path_regex` — оно есть в редакторе
правил. Список кэшируется на две минуты. Решения: `docs/adr/011-macos-application-rules.md`,
`docs/adr/013-windows-application-rules.md`.
...
- `docs/adr/` — принятые решения (Wails v2, архитектура Go, ревизии, supervisor,
  ...
  значок в менюбаре, правила по приложениям на macOS и Windows, остановка чужого ядра).
