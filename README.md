...
каждой две кнопки — «Через VPN» и «Напрямую». Кнопка добавляет обычное правило
маршрутизации, которое выбирает программу по процессу, а не по адресу: на macOS это
выражение по пути приложения (`.app`), на Windows — имя файла программы (`process_name`),
поэтому правило переживает обновление, которое переносит программу в другую папку.
Список на Windows собирается из запущенных программ и ярлыков меню «Пуск»; сама Windows,
её установщики и сама SingBoxUI в него не попадают. Целое дерево каталогов (например,
игровую библиотеку) удобнее исключать условием `process_path_regex` — оно есть в редакторе
